...
namespace Common.Http
{
    public class MyHttpClient
    {
        ...
        #region properties
        public int DefaultConnectionLimit = 100;
        public int DefaultTimeOut = 1000 * 100;
        ...
        public MyHttpClient(string url, HttpClientContext context)
        ...
        public MyHttpClient(string url, HttpClientContext context, bool keepContext)
        {
            this.url = url;
        ...
        public byte[] GetBytes(HttpWebRequest req)
        {
        ...
        /// <returns></returns>
        public string UploadString(string UploadText)
        {
            ...
            {
                foreach (var v in responseHeaders.AllKeys)
                {
                    client.Headers[v] = responseHeaders[v];
                }
            }
            client.Encoding = defaultEncoding;
            client.Proxy = null;

            string strResult = client.UploadString(url, UploadText);
            return strResult;
        }
        ...
        /// <returns></returns>
        public string GetCurrentInput(System.Web.HttpContext context, Encoding encode = null)
        {
            if (encode == null)
                encode = Encoding.UTF8;
            string strInput = StreamHelper.StreamToString(context.Request.InputStream, encode);
            return strInput;
        }
    }
